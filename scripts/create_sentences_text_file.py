"""This module implement a function that create
    a text file from providence csvs data."""
...
def create_sentences_file(csvs_directory: str,
    ...
    - out_directory : str
        ...
        will be stored.
    """
    # we only consider mother and father as adult utterances.
    adults: List[str]=["Mother", "Father"]
    input_directory = Path(csvs_directory)
    output_directoty = Path(out_directory)
    output_directoty.mkdir(parents=True, exist_ok=True)
    output_file = output_directoty / "providence.txt"
